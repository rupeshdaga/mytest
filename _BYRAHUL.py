...
def user_input (ipam_code):
    ...
    for item in amdb:
        if item ['IPAM_Code']== ipam_code:
            if item ['Network_Function'] == 'Access Switch' and item ['Vendor'] == 'H3C' :
    ...
    for item in amdb:
        if item ['IPAM_Code']== ipam_code:
            if item ['Network_Function'] == 'Distribution Switch' and item ['Vendor'] == 'H3C' :
                ip_hcl_HP.append( item ['IP_Address'] )

    for item in amdb :
        if item ['IPAM_Code']== ipam_code :
            if item ['Network_Function'] == 'Core Switch' and item ['Vendor'] == 'H3C' :
                ip_hcl_HP.append( item ['IP_Address'] )
    for item in amdb :
        if item ['IPAM_Code'] == ipam_code :
            if item ['Network_Function'] == 'Internet Switch' and item ['Vendor'] == "H3C" :
                ip_hcl_HP.append ( item ['IP_Address'] )

    for item in amdb :
        if item ['IPAM_Code']== ipam_code :
            if item ['Network_Function'] == 'Internet Switch' and item ['Vendor'] == "Cisco" :
                ip_hcl_cisco.append( item ['IP_Address'] )


    for item in amdb:
        if item ['IPAM_Code']== ipam_code:
            if item ['Network_Function'] == 'Access Switch' and item ['Vendor'] == "Cisco" :
                ip_hcl_cisco.append( item ['IP_Address'] )

    for item in amdb:
        if item ['IPAM_Code']== ipam_code:
            if item ['Network_Function'] == 'Distribution Switch' and item ['Vendor'] == "Cisco" :
                ip_hcl_cisco.append( item ['IP_Address'] )
    for item in amdb :
        if item ['IPAM_Code']== ipam_code :
            if item ['Network_Function'] == 'Core Switch' and item ['Vendor'] == "Cisco" :
                ip_hcl_cisco.append ( item ['IP_Address'] )

    for item in amdb :
        if item ['IPAM_Code'] == ipam_code :
            if item ['Network_Function'] == 'Internet Switch' and item ['Vendor'] == "HPE" :
                ip_hcl_HPE.append ( item ['IP_Address'] )

    for item in amdb :
        if item ['IPAM_Code'] == ipam_code :
            if item ['Network_Function'] == 'Access Switch' and item ['Vendor'] == "HPE" :
                ip_hcl_HPE.append ( item ['IP_Address'] )

    for item in amdb :
        if item ['IPAM_Code'] == ipam_code :
            if item ['Network_Function'] == 'Distribution Switch' and item ['Vendor'] == "HPE" :
                ip_hcl_HPE.append ( item ['IP_Address'] )
    for item in amdb :
        if item ['IPAM_Code'] == ipam_code :
            if item ['Network_Function'] == 'Core Switch' and item ['Vendor'] == "HPE" :
                ip_hcl_HPE.append ( item ['IP_Address'] )
    ...
    if len (ip_hcl_cisco) > 0:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as mainexecutor:
            for vip in ip_hcl_cisco :
                # VERSION(vip)
                mainexecutor.submit(VERSION,vip,xyz)
    qdata = []
    while not xyz.empty():
...
def VERSION (ipsw, pipe):
        ...
        for retry in range (5):
            try:
                ...
                hardware = session.send_command ('dir flash:')

                ip_dict = {'IP_Addr' : ipsw , 'model_number' : output [0]['hardware'][0], 'running_version' : output[0]['version'], 'WAS_LOGIN_SUCCESSFUL': True, 'MESSAGE_FOR_SELF' : 'TEXTFSM failed'}
                # ou.append(ip_dict)
                session.disconnect( )
                pipe.put(ip_dict)
                return
            ...
            ip_dict={ 'IP_Addr' : ipsw,'model_number' : '-',
                      'running_version' : '-','WAS_LOGIN_SUCCESSFUL' : False }
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if ipam_code in code :
            ...
            data_from_threads = user_input(ipam_code)
            # pprint(pid_sw)
            break
        else :
